import electron, { ipcMain as ipcMain$1, app as app$1, screen, BrowserWindow } from "electron";
...
function stringToUint8Array$1(string) {
  assertString(string);
  ...
  */
  ...
  _decryptData(data) {
    ...
    try {
      ...
      const password = crypto$1.pbkdf2Sync(this.#encryptionKey, initializationVector, 1e4, 32, "sha512");
      const decipher = crypto$1.createDecipheriv(encryptionAlgorithm, password, initializationVector);
      const slice = data.slice(17);
      const dataUpdate = typeof slice === "string" ? stringToUint8Array$1(slice) : slice;
  ...
  _write(value) {
    ...
    if (this.#encryptionKey) {
      ...
      data = concatUint8Arrays([initializationVector, stringToUint8Array$1(":"), cipher.update(stringToUint8Array$1(data)), cipher.final()]);
    }
      ...
      __disposeResources2 = function(env2) {
        ...
        function next2() {
          while (r = env2.stack.pop()) {
            try {
              if (!r.async && s === 1) return s = 0, env2.stack.push(r), Promise.resolve().then(next2);
              if (r.dispose) {
...
var PostgrestFilterBuilder = class extends PostgrestTransformBuilder {
  ...
  */
  regexMatch(column, pattern2) {
    this.url.searchParams.append(column, `match.${pattern2}`);
    return this;
  }
  /**
  * Match only rows where `column` matches the PostgreSQL regex `pattern`
  ...
  * @param pattern - The PostgreSQL regular expression pattern to match with
  */
  regexIMatch(column, pattern2) {
    this.url.searchParams.append(column, `imatch.${pattern2}`);
    return this;
  }
  /**
...
class Serializer {
  ...
  _encodeUserBroadcastPush(message, encodingType, encodedPayload) {
    ...
    const metadata2 = Object.keys(rest).length === 0 ? "" : JSON.stringify(rest);
    ...
    if (metadata2.length > 255) {
      throw new Error(`metadata length ${metadata2.length} exceeds maximum of 255`);
    }
    const metaLength = this.USER_BROADCAST_PUSH_META_LENGTH + joinRef.length + ref2.length + topic.length + userEvent.length + metadata2.length;
    ...
    view.setUint8(offset++, metadata2.length);
    view.setUint8(offset++, encodingType);
    Array.from(joinRef, (char) => view.setUint8(offset++, char.charCodeAt(0)));
    Array.from(ref2, (char) => view.setUint8(offset++, char.charCodeAt(0)));
    Array.from(topic, (char) => view.setUint8(offset++, char.charCodeAt(0)));
    Array.from(userEvent, (char) => view.setUint8(offset++, char.charCodeAt(0)));
    Array.from(metadata2, (char) => view.setUint8(offset++, char.charCodeAt(0)));
  ...
  _decodeUserBroadcast(buffer, view, decoder) {
    ...
    const metadata2 = decoder.decode(buffer.slice(offset, offset + metadataSize));
    ...
    if (metadataSize > 0) {
      data["meta"] = JSON.parse(metadata2);
    }
...
const convertCell = (type2, value) => {
  if (type2.charAt(0) === "_") {
    const dataType2 = type2.slice(1, type2.length);
    return toArray(value, dataType2);
  }
  switch (type2) {
    ...
    if (["insert", "update", "delete"].includes(typeLower)) {
      ...
      (_b = this.bindings[typeLower]) === null || _b === void 0 ? void 0 : _b.filter((bind) => {
        ...
        if (typeof handledPayload === "object" && "ids" in handledPayload) {
          ...
          const { schema, table, commit_timestamp, type: type3, errors: errors2 } = postgresChanges;
...
var NamespaceOperations = class {
  ...
  async createNamespace(id2, metadata2) {
    const request = {
      namespace: id2.namespace,
      properties: metadata2?.properties
    };
    const response = await this.client.request({
      method: "POST",
      path: `${this.prefix}/namespaces`,
  ...
  async createNamespaceIfNotExists(id2, metadata2) {
    try {
      return await this.createNamespace(id2, metadata2);
    } catch (error) {
...
var IcebergRestCatalog = class {
   ...
   */
  async createNamespace(id2, metadata2) {
    return this.namespaceOps.createNamespace(id2, metadata2);
  }
  /**
   ...
   */
  async createNamespaceIfNotExists(id2, metadata2) {
    return this.namespaceOps.createNamespaceIfNotExists(id2, metadata2);
  }
  /**
...
async function _handleRequest$1(fetcher, method, url, options, parameters, body, namespace) {
  return new Promise((resolve2, reject) => {
    fetcher(url, _getRequestParams$1(method, options, parameters, body)).then((result) => {
      ...
      return result.json();
    }).then((data) => resolve2(data)).catch((error) => handleError$1(error, reject, options, namespace));
  });
}
  ...
  async uploadOrUpdate(method, path2, fileBody, fileOptions) {
    ...
    return _this.handleOperation(async () => {
      ...
      if (typeof Blob !== "undefined" && fileBody instanceof Blob) {
        ...
        if (metadata2) body.append("metadata", _this.encodeMetadata(metadata2));
        body.append("", fileBody);
      } else if (typeof FormData !== "undefined" && fileBody instanceof FormData) {
        body = fileBody;
        if (!body.has("cacheControl")) body.append("cacheControl", options.cacheControl);
        if (metadata2 && !body.has("metadata")) body.append("metadata", _this.encodeMetadata(metadata2));
      } else {
        body = fileBody;
        headers["cache-control"] = `max-age=${options.cacheControl}`;
        headers["content-type"] = options.contentType;
        if (metadata2) headers["x-metadata"] = _this.toBase64(_this.encodeMetadata(metadata2));
...
class CustomAuthError extends AuthError {
  constructor(message, name, status, code2) {
    super(message, status, code2);
    this.name = name;
    this.status = status;
  }
...
class GoTrueAdminApi {
   ...
   */
  async signOut(jwt, scope2 = SIGN_OUT_SCOPES[0]) {
    if (SIGN_OUT_SCOPES.indexOf(scope2) < 0) {
      throw new Error(`@supabase/auth-js: Parameter scope must be one of ${SIGN_OUT_SCOPES.join(", ")}`);
    }
    try {
      await _request(this.fetch, "POST", `${this.url}/logout?scope=${scope2}`, {
        headers: this.headers,
        jwt,
        noResolveJson: true
      });
...
class WebAuthnError extends Error {
  constructor({ message, code: code2, cause, name }) {
    var _a;
    super(message, { cause });
...
class GoTrueClient {
  ...
  async _signOut({ scope: scope2 } = { scope: "global" }) {
    return await this._useSession(async (result) => {
      var _a;
      ...
      if (accessToken) {
        const { error } = await this.admin.signOut(accessToken, scope2);
        if (error) {
...
ipcMain$1.handle("get-secrets", () => store.get("secrets", {}));
ipcMain$1.handle("save-secrets", (_, secrets) => {
  store.set("secrets", secrets);
  return { success: true };
});
ipcMain$1.handle("supabase-sync", async () => {
...
ipcMain$1.handle("cloudflare-sync", async () => {
  const secrets = store.get("secrets", {});
